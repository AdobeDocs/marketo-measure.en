...
Before we get into specifics of each platform, we'll go over how to connect any of these accounts to Marketo Measure. First log into the Marketo Measure app and navigate to the **[!UICONTROL Settings]** option under the **[!UICONTROL My Account]** tab at the top left of the screen. Next, select **[!UICONTROL Connections]** under the **[!UICONTROL Integrations]** section on the left.

As shown in the image below, you will see a button to set up new ads connections.

![](assets/2.png)

After you click the [!UICONTROL Set up New Ads Connection] button, a window (shown below) will pop up with four ad [!UICONTROL connect]ion types. Click connect and another window will appear asking for credentials. Enter the credentials and click [!UICONTROL authorize] to connect the account to Marketo Measure.
...
Marketo Measure Searches for Tracking Templates in your [!DNL AdWords] account:
...
Learn more about our [[!DNL AdWords] auto-tagging functionality](/help/api-connections/utilizing-marketo-measures-api-connections/understanding-marketo-measure-adwords-tagging.md).
...
1. Go to My Account >[!UICONTROL Settings] > Integrations > Connections.
...
1. In the top right-hand corner, toggle the **Autotagging** switch to **Yes**. At the bottom of the page, click **[!UICONTROL Learn More»]** to expand the text box and click **Save**. Auto-tagging setup is complete.
...
Auto-tagging should be enabled through [!DNL Marketo Measure] so that the custom Marketo Measure parameters can be automatically appended. There is no risk of losing past ad performance history with Bing Ads.

Visit the [[!DNL Bing Ads]](https://advertise.bingads.microsoft.com/en-us/blog/post/august-2016/upgraded-urls-now-available-in-bing-ads-an-easier-way-to-manage-your-tracking-urls) website for more information on adding tags on their platform.

## Facebook Ads {#facebook-ads}

Marketo Measure’s integration with [!DNL Facebook] allows it to automatically download ad information and tag the URL with its parameters. [!DNL Marketo Measure] will pull in the Campaign and Ad Set information through our auto-tagging. The Ad Set will populate our Ad Group Name field. For more information on setting up URL tags on the [!DNL Facebook] platform, visit the [!DNL Facebook] [business](https://www.facebook.com/business/help/1016122818401732/?ref=u2u) page.

Before enabling auto-tagging with [!DNL Facebook Ads], it is important to export the previous performance history as a CSV. At this point, when [!DNL Marketo] Measure tags [!DNL Facebook Ads] with its _bf parameter, Facebook reads the ads as brand new and erases the performance history. Therefore it is important to export a record of the previous performance if that is something of value to you and your organization.

Please note that you may connect your Facebook account at any time to the [!DNL Marketo] Measure app and no data will be lost--it is only when auto-tagging is enabled that the performance history is wiped.
...
For platforms that are not integrated with [!DNL Marketo Measure], [!DNL Marketo Measure’s] auto-tagging functionality cannot be used. The parameters will need to be added manually.
